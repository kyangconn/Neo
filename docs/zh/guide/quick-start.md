...
2. 向下滚动到 **"DeepSeek API"** 部分。
3. 将你的 API Key **粘贴**到文本框中。
4. 点击 **"Save DeepSeek Profile"**——你应该会看到一条成功消息。
5. 点击 **"Test Connection"** 验证一切是否正常。出现绿色对勾 ✅ 表示一切就绪。

![API 设置页面](../images/settings-api.png)

> **📸 需要截图：** 输入 API Key 后，截取设置页面中已填写 Key 的 DeepSeek API 部分。图片保存为 `docs/images/settings-api.png`。
...
3. 填写角色详细信息：
   ...
   | **First Message** | 角色说的开场白 | *"欢迎你，旅人。我已等候多时。这些书几天来一直在低语你的名字。"* |
...
> **📸 需要截图：** 向你的角色发送一条问候（例如 *"你好！这是什么地方？"*），截取显示你消息和 AI 回复的聊天视图。图片保存为 `docs/images/chat-first-message.png`。
