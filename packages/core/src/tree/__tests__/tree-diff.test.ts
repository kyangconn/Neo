...
describe('diffTreesById', () => {
  ...
  it('identifies messages only in tree A', () => {
    const a = [
      makeMsg({ id: '1', content: 'A only' }),
      makeMsg({ id: '2', content: 'Shared' }),
    ]
  ...
  it('identifies messages only in tree B', () => {
    ...
    const b = [
      makeMsg({ id: '1', content: 'Shared' }),
      makeMsg({ id: '2', content: 'B only' }),
    ]
    const result = diffTreesById(a, b)
    expect(result.shared).toHaveLength(1)
    expect(result.onlyInA).toHaveLength(0)
  ...
  it('detects divergence points when B has children under shared parent', () => {
    const a = [
      ...
      makeMsg({ id: '1', content: 'A child', parentId: 'root' }),
    ]
    const b = [
      makeMsg({ id: 'root', content: 'Root' }),
      makeMsg({ id: '2', content: 'B child', parentId: 'root' }),
    ]
    const result = diffTreesById(a, b)
  ...
  it('does not report divergence when parent is also only in B', () => {
    ...
    const b = [
      makeMsg({ id: '2', content: 'B root' }),
      makeMsg({ id: '3', content: 'B child', parentId: '2' }),
    ]
    const result = diffTreesById(a, b)
...
describe('diffTreesByContent', () => {
  ...
  it('identifies current messages not matched as onlyInA', () => {
    const current = [
      makeMsg({ id: 'old-1', content: 'Matched' }),
      makeMsg({ id: 'old-2', content: 'Unmatched' }),
    ]
  ...
  it('handles multiple messages with the same fingerprint', () => {
    const current = [
      makeMsg({ id: 'old-1', content: 'Repeat' }),
      makeMsg({ id: 'old-2', content: 'Repeat' }),
    ]
    const incoming = [
      makeMsg({ id: 'new-1', content: 'Repeat' }),
      makeMsg({ id: 'new-2', content: 'Repeat' }),
    ]
    const result = diffTreesByContent(current, incoming)
  ...
  it('handles completely disjoint trees with no overlap', () => {
    const current = [
      makeMsg({ id: '1', content: 'Alpha' }),
      makeMsg({ id: '2', content: 'Beta' }),
    ]
    const incoming = [
      makeMsg({ id: '3', content: 'Gamma' }),
      makeMsg({ id: '4', content: 'Delta' }),
    ]
    const result = diffTreesByContent(current, incoming)
    expect(result.shared).toHaveLength(0)
    expect(result.onlyInA).toHaveLength(2)
  ...
  it('matches by role and content concurrently', () => {
    ...
    const incoming = [
      makeMsg({ id: '3', role: 'user', content: 'Hello' }),
    ]
    const result = diffTreesByContent(current, incoming)
