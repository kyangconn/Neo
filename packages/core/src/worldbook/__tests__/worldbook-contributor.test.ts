...
describe("resolveWorldbookEntries", () => {
  ...
  it("filters out disabled entries", () => {
    const entries = [makeEntry({ id: "always-on", type: "always", enabled: false })];
    const result = resolveWorldbookEntries(entries, "test", emptyRecent);
    expect(result.matched).toHaveLength(0);
  });

  it("returns always entries even without keyword match", () => {
    const entries = [makeEntry({ id: "always-1", title: "Always lore", type: "always", content: "Always present" })];
  ...
  it("scans recent messages for keywords", () => {
    const entries = [makeEntry({ id: "rec-1", title: "Recall", type: "trigger", keys: "artifact", triggerMode: "or" })];
    const recentMessages: Message[] = [
      {
        id: "m1",
        chatId: "c1",
