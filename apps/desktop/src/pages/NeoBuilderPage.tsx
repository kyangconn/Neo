...
import { ArrowLeft, BookOpen, Bot, Brain, CheckCircle2, ChevronDown, ChevronRight, CircleDashed, Eye, FileText, Globe2, Loader2, Save, Send, Sparkles, Trash2, User, Wrench, XCircle } from 'lucide-react'
import { Button, Dialog, DialogContent, DialogDescription, DialogHeader, DialogTitle, Textarea } from '@neo-tavern/ui'
import { generateId } from '@neo-tavern/shared'
import type { Character, CreateCharacterInput, CreateWorldbookEntryInput, MessageUsage, Worldbook } from '@neo-tavern/shared'
...
function normalizeRestoredMessages(messages: unknown): BuilderMessage[] {
  ...
  return messages
    .filter((message): message is BuilderMessage => (
      !!message
...
function buildWorkspaceTitle(snapshot: BuilderWorkspaceSnapshot) {
  ...
  if (latestUserMessage) return latestUserMessage.length > 24 ? `${latestUserMessage.slice(0, 24)}...` : latestUserMessage
  return '未命名创作'
}

function createWorkspaceRecord(snapshot: BuilderWorkspaceSnapshot, updatedAt = new Date().toISOString()): BuilderWorkspaceRecord {
  return {
    ...snapshot,
...
function normalizeWorkspaceRecord(record: Partial<BuilderWorkspaceRecord>): BuilderWorkspaceRecord | null {
  const builderSessionId = typeof record.builderSessionId === 'string' ? record.builderSessionId : typeof record.id === 'string' ? record.id : generateId()
...
function initialMessages(): BuilderMessage[] {
  return [{
    id: generateId(),
    role: 'assistant',
    content: '把角色想法直接丢给我就行。我会按 Whale Play 工作流先对齐规划，信息不够时给你几个选项；需要真实资料时可以打开联网搜索。',
    choices: [
...
function ToolTimeline({ events }: { events: NeoBuilderToolEvent[] | undefined }) {
  ...
  return (
    <div className="relative pb-3 pl-5">
      <span className={`absolute left-[-6px] top-1 flex h-3 w-3 items-center justify-center rounded-full bg-background ${
        hasError ? 'text-destructive' : hasRunning ? 'text-primary' : 'text-emerald-500'
      }`}>
        {hasError ? <XCircle className="h-3.5 w-3.5" /> : hasRunning ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <CheckCircle2 className="h-3.5 w-3.5" />}
      </span>
...
function BuilderActivityTimeline({ message }: { message: BuilderMessage }) {
  ...
  const elapsed = message.startedAt
    ? formatElapsed((message.completedAt ?? now) - message.startedAt)
    : null
    ...
    <aside className="flex min-h-0 min-w-0 flex-col overflow-hidden rounded-lg border bg-card">
      <div className="shrink-0 border-b p-3">
        <div className="flex items-center gap-2 text-sm font-semibold">
          <FileText className="h-4 w-4" />创作记录
        </div>
      </div>
            ...
            >
              ...
              >
                ...
                <div className="mt-1 flex min-w-0 items-center justify-between gap-2 text-xs">
                  <span className={`shrink-0 ${getWorkspaceRecordStatusClass(record)}`}>{getWorkspaceRecordStatus(record)}</span>
                  <span className="min-w-0 truncate text-muted-foreground">{formatCharacterUpdatedAt(record.updatedAt)}</span>
                </div>
              </button>
              <button
                type="button"
...
function BuilderChatMessage({
  ...
  return (
    <div className={`flex min-w-0 gap-3 ${isUser ? 'justify-end' : 'justify-start'}`}>
      ...
      <div className={`min-w-0 overflow-hidden ${isUser ? 'max-w-[min(82%,48rem)] rounded-lg border bg-primary p-4 text-primary-foreground' : 'w-full max-w-4xl py-1'}`}>
        ...
        {message.choices?.length ? (
          <div className="mt-3 flex min-w-0 flex-wrap gap-2">
            {message.choices.map((choice) => (
              <Button key={choice.id} type="button" variant="outline" size="sm" className="max-w-full whitespace-normal break-words text-left [overflow-wrap:anywhere]" onClick={() => onChoice(choice)}>
                {choice.label}
              </Button>
            ))}
          </div>
...
export function NeoBuilderPage() {
  ...
  const [worldbookDraft, setWorldbookDraft] = useState<WorldbookDraft | null>(() => initialSnapshot?.worldbookDraft ?? null)
  const [creationPlan, setCreationPlan] = useState<NeoCreationPlan | null>(() => initialSnapshot?.creationPlan ?? null)
  const [personalityPalette, setPersonalityPalette] = useState<NeoPersonalityPalette | null>(() => initialSnapshot?.personalityPalette ?? null)
  const [evaluationReport, setEvaluationReport] = useState<NeoBuilderEvaluationReport | null>(() => initialSnapshot?.evaluationReport ?? null)
  const [artifactView, setArtifactView] = useState<ArtifactView>(null)
  const [savedCharacterId, setSavedCharacterId] = useState<string | null>(() => initialSnapshot?.savedCharacterId ?? null)
  const [workspaceRecords, setWorkspaceRecords] = useState<BuilderWorkspaceRecord[]>(() => readInitialBuilderRecords(initialSnapshot ?? null))
  ...
  }, [targetId, messages, input, webSearchEnabled, lastResult, draft, worldbookDraft, creationPlan, personalityPalette, evaluationReport, savedCharacterId, builderSessionId])
  ...
  const applyDraftFromResult = (result: NeoBuilderTurnResult) => {
    ...
    setWorldbookDraft(result.draft.worldbookEntries.length > 0
      ? {
  ...
  const sendMessage = async (content: string, webSearchOverride = webSearchEnabled) => {
    ...
    const assistantMessage: BuilderMessage = { id: assistantId, role: 'assistant', content: '', pending: true, startedAt: Date.now() }
    ...
    try {
      ...
      setMessages((prev) => prev.map((item) => (
        item.id === assistantId
          ? { ...item, content: message, pending: false, completedAt: Date.now() }
          : item
      )))
      toast('error', message)
    } finally {
  ...
  const hasCreationPlan = allToolEvents.some((event) => event.name === 'present_creation_plan' && event.status === 'done')
  const hasWebSearch = allToolEvents.some((event) => event.name === 'web_search' && event.status === 'done')
  const hasSavedDraftTool = allToolEvents.some((event) => event.name === 'save_character_draft' && event.status === 'done')
  const planEntries = creationPlan?.entries ?? []
  const completedPlanEntries = planEntries.filter((entry) => entry.status === 'done' || entry.status === 'skipped').length
  const steps = [
    { label: '收集方向', done: hasUserMessage, active: running && !hasUserMessage },
    { label: '对齐规划', done: !!creationPlan || hasOptionPrompt || hasCreationPlan || !!draft, active: running && hasUserMessage && !creationPlan && !draft },
    { label: '检索参考资料', done: hasWebSearch, active: running && webSearchEnabled && !hasWebSearch, optional: true },
    { label: '逐条产出', done: planEntries.length ? completedPlanEntries === planEntries.length : !!draft || hasSavedDraftTool, active: running && (!!creationPlan || hasUserMessage) && !draft },
    ...
    <div className="flex h-full min-h-0 min-w-0 flex-col overflow-hidden">
        ...
        <div className="flex min-w-0 items-center justify-between gap-4">
          ...
          <div className="flex shrink-0 items-center gap-2">
            <Button variant="outline" size="sm" onClick={() => navigate('/character')}>
              <ArrowLeft className="mr-1 h-3.5 w-3.5" />Characters
            </Button>
          </div>
        </div>
      </div>
      ...
      <div className="grid min-h-0 min-w-0 flex-1 grid-cols-1 gap-4 overflow-hidden p-4 xl:grid-cols-[minmax(0,1fr)_minmax(340px,380px)] 2xl:grid-cols-[minmax(0,1fr)_430px]">
        <div className="grid min-h-0 min-w-0 grid-cols-1 gap-4 overflow-hidden lg:grid-cols-[230px_minmax(0,1fr)] 2xl:grid-cols-[260px_minmax(0,1fr)]">
          ...
          {error && (
            <div className="mx-5 mb-3 rounded-md bg-destructive/10 p-3 text-sm text-destructive">
              {error}
            </div>
          )}
            ...
            <div className="mx-auto w-full min-w-0 max-w-4xl">
              <div className="mb-2 flex flex-wrap items-center gap-2">
                <Button
                  ...
                  disabled={running}
                >
                  <Globe2 className="mr-1 h-3.5 w-3.5" />联网搜索
                </Button>
                {lastResult?.draft && (
                  <span className="inline-flex items-center gap-1 text-xs text-emerald-500">
                    <CheckCircle2 className="h-3.5 w-3.5" />产出物已就绪
                  </span>
                )}
              </div>
              <div className="flex min-w-0 items-end gap-2">
                ...
                />
                <Button type="button" size="icon" className="h-10 w-10 shrink-0" onClick={() => sendMessage(input)} disabled={running || saving || !input.trim()}>
          ...
          <div className="min-h-0 min-w-0 flex-1 overflow-y-auto overflow-x-hidden p-4">
            <section>
              <div className="mb-3 flex items-center gap-2 text-sm font-semibold">
                <CheckCircle2 className="h-4 w-4" />进度
              </div>
              <div className="space-y-2">
                {steps.map((step, index) => (
                  <div key={step.label} className={`flex items-center gap-3 rounded-md border bg-background p-3 ${
                    ...
                    <div className={`flex h-6 w-6 shrink-0 items-center justify-center rounded-full text-xs ${
                      ...
                      {step.done ? <CheckCircle2 className="h-3.5 w-3.5" /> : step.active ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : index + 1}
                    </div>
                    <div className="min-w-0">
            ...
            <section className="mt-5">
              <div className="mb-3 flex items-center gap-2 text-sm font-semibold">
                <Sparkles className="h-4 w-4" />产出物
              </div>
              <div className="space-y-3">
                  ...
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0">
                      <div className="flex items-center gap-2 text-sm font-medium">
                        <FileText className="h-4 w-4" />创作规划.yaml
                      </div>
                ...
                <div className="rounded-md border bg-background p-3">
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0">
                      <div className="flex items-center gap-2 text-sm font-medium">
                        <Brain className="h-4 w-4" />性格调色盘
                      </div>
                      <p className="mt-1 truncate text-xs text-muted-foreground">
                        {personalityPalette ? `${personalityPalette.base || '无底色'} · ${personalityPalette.derivatives.length} 组衍生` : '尚未生成'}
                      </p>
                    </div>
                    <Button size="sm" variant="outline" onClick={() => setArtifactView('palette')} disabled={!personalityPalette}>
                      <Eye className="mr-1 h-3.5 w-3.5" />查看
                    </Button>
                  </div>
                </div>
                ...
                <div className="rounded-md border bg-background p-3">
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0">
                      <div className="flex items-center gap-2 text-sm font-medium">
                        <FileText className="h-4 w-4" />角色卡
                      </div>
                      <p className="mt-1 truncate text-xs text-muted-foreground">
                        {draft ? draft.name : '尚未生成'}
                      </p>
                    </div>
                ...
                <div className="rounded-md border bg-background p-3">
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0">
                      <div className="flex items-center gap-2 text-sm font-medium">
                        <BookOpen className="h-4 w-4" />世界书
                      </div>
                      <p className="mt-1 truncate text-xs text-muted-foreground">
                        {worldbookDraft?.entries.length ? `${worldbookDraft.entries.length} 条细则` : '尚未生成'}
                      </p>
                    </div>
                    <Button size="sm" variant="outline" onClick={() => setArtifactView('worldbook')} disabled={!worldbookDraft?.entries.length}>
                      <Eye className="mr-1 h-3.5 w-3.5" />查看
                    </Button>
                  </div>
                </div>
                ...
                <div className="rounded-md border bg-background p-3">
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0">
                      <div className="flex items-center gap-2 text-sm font-medium">
                        <CheckCircle2 className="h-4 w-4" />评估报告
                      </div>
                      <p className="mt-1 truncate text-xs text-muted-foreground">
                    ...
                    <div className="flex shrink-0 gap-2">
                      <Button size="sm" variant="outline" onClick={() => setArtifactView('evaluation')} disabled={!evaluationReport}>
                        <Eye className="mr-1 h-3.5 w-3.5" />查看
                      </Button>
                    </div>
                  </div>
                ...
                <Button type="button" variant="outline" className="w-full" onClick={() => sendMessage('请评估当前 Whale Builder 草稿，检查角色卡、性格调色盘、世界书条目、创作规划.yaml 是否完整，并给出可执行修改建议。')} disabled={running || saving || (!draft && !creationPlan)}>
                  <CheckCircle2 className="mr-1 h-4 w-4" />评估当前草稿
                </Button>

                <Button className="w-full" onClick={handleSave} disabled={!draft?.name.trim() || running || saving || !!savedCharacterId}>
                  <Save className="mr-1 h-4 w-4" />{savedCharacterId ? '已保存' : saving ? '保存中...' : '创建'}
                </Button>
              </div>
            </section>
          </div>
      ...
      <Dialog open={artifactView === 'plan'} onOpenChange={(open: boolean) => { if (!open) setArtifactView(null) }}>
      ...
      <Dialog open={artifactView === 'palette'} onOpenChange={(open: boolean) => { if (!open) setArtifactView(null) }}>
        <DialogContent className="max-h-[85vh] max-w-3xl overflow-y-auto">
          <DialogHeader>
            ...
            <div className="space-y-4 text-sm">
              <div className="grid gap-3 sm:grid-cols-3">
                ...
                <section className="rounded-md border bg-background p-3">
                  ...
                  <p className="mt-2 break-words [overflow-wrap:anywhere]">{personalityPalette.main.join('、') || '-'}</p>
                </section>
                <section className="rounded-md border bg-background p-3">
                  <h3 className="text-xs font-semibold text-muted-foreground">点缀</h3>
                  <p className="mt-2 break-words [overflow-wrap:anywhere]">{personalityPalette.accents.join('、') || '-'}</p>
                </section>
              </div>
                ...
                <section key={derivative.color} className="rounded-md border bg-background p-4">
                  ...
                  <div className="space-y-2">
                    {derivative.items.map((item, index) => (
                      <p key={`${derivative.color}-${index}`} className="whitespace-pre-wrap break-words text-sm leading-relaxed [overflow-wrap:anywhere]">
                        {index + 1}. {item}
                      </p>
                    ))}
                  </div>
                ...
                <section className="rounded-md border bg-background p-4">
                  ...
                  <div className="space-y-2">
                    {personalityPalette.futureDerivatives.map((item, index) => (
                      <p key={`${item}-${index}`} className="whitespace-pre-wrap break-words text-sm leading-relaxed [overflow-wrap:anywhere]">{item}</p>
                    ))}
                  </div>
                </section>
              ) : null}
              {personalityPalette.compiledText ? (
                <section>
                  ...
                  <p className="whitespace-pre-wrap break-words rounded-md border bg-muted/30 p-3 text-sm leading-relaxed [overflow-wrap:anywhere]">{personalityPalette.compiledText}</p>
                </section>
              ) : null}
            </div>
          ) : null}
      ...
      <Dialog open={artifactView === 'evaluation'} onOpenChange={(open: boolean) => { if (!open) setArtifactView(null) }}>
        <DialogContent className="max-h-[85vh] max-w-3xl overflow-y-auto">
          <DialogHeader>
          ...
          {evaluationReport ? (
            <div className="space-y-4 text-sm">
              <section className="rounded-md border bg-background p-4">
                ...
                <p className="whitespace-pre-wrap break-words leading-relaxed [overflow-wrap:anywhere]">{evaluationReport.summary}</p>
                {typeof evaluationReport.score === 'number' ? <p className="mt-2 text-xs text-muted-foreground">Score {evaluationReport.score}/100</p> : null}
              </section>
              <section className="rounded-md border bg-background p-4">
              ...
              <section className="rounded-md border bg-background p-4">
                ...
                <div className="space-y-2">
                  {evaluationReport.suggestions.map((item, index) => (
                    <p key={`${item}-${index}`} className="whitespace-pre-wrap break-words [overflow-wrap:anywhere]">{index + 1}. {item}</p>
                  ))}
                </div>
              </section>
            </div>
      ...
      <Dialog open={artifactView === 'character'} onOpenChange={(open: boolean) => { if (!open) setArtifactView(null) }}>
        <DialogContent className="max-h-[85vh] max-w-3xl overflow-y-auto">
          <DialogHeader>
          ...
          {draft ? (
            <div className="space-y-5 text-sm">
              ...
              <section>
                ...
                <p className="whitespace-pre-wrap break-words leading-relaxed [overflow-wrap:anywhere]">{draft.description || '-'}</p>
              </section>
              <section>
                <h3 className="mb-2 text-xs font-semibold uppercase text-muted-foreground">Personality</h3>
                <p className="whitespace-pre-wrap break-words leading-relaxed [overflow-wrap:anywhere]">{draft.personality || '-'}</p>
              </section>
              <section>
                <h3 className="mb-2 text-xs font-semibold uppercase text-muted-foreground">Scenario</h3>
                <p className="whitespace-pre-wrap break-words leading-relaxed [overflow-wrap:anywhere]">{draft.scenario || '-'}</p>
              </section>
              <section>
                <h3 className="mb-2 text-xs font-semibold uppercase text-muted-foreground">First Message</h3>
                <p className="whitespace-pre-wrap break-words rounded-md border bg-muted/30 p-3 leading-relaxed [overflow-wrap:anywhere]">{draft.firstMessage || '-'}</p>
              </section>
              <section>
                <h3 className="mb-2 text-xs font-semibold uppercase text-muted-foreground">Example Dialogues</h3>
                <p className="whitespace-pre-wrap break-words rounded-md border bg-background p-3 font-mono text-xs leading-relaxed [overflow-wrap:anywhere]">{draft.exampleDialogues || '-'}</p>
              </section>
            </div>
          ) : null}
        </DialogContent>
      ...
      <Dialog open={artifactView === 'worldbook'} onOpenChange={(open: boolean) => { if (!open) setArtifactView(null) }}>
        <DialogContent className="max-h-[85vh] max-w-3xl overflow-y-auto">
          <DialogHeader>
              ...
              {worldbookDraft.entries.map((entry, index) => (
                <section key={`${entry.title}-${index}`} className="rounded-md border bg-background p-4">
                  <div className="flex flex-wrap items-start justify-between gap-3">
                    <h3 className="min-w-0 break-words text-sm font-semibold [overflow-wrap:anywhere]">{entry.title}</h3>
                  ...
                  {entry.keys ? (
                    <p className="mt-3 break-words text-xs text-muted-foreground [overflow-wrap:anywhere]">Keys: {entry.keys}</p>
                  ) : null}
                  {entry.secondaryKeys ? (
                    <p className="mt-1 break-words text-xs text-muted-foreground [overflow-wrap:anywhere]">Secondary: {entry.secondaryKeys}</p>
                  ) : null}
                  <p className="mt-3 whitespace-pre-wrap break-words text-sm leading-relaxed [overflow-wrap:anywhere]">{entry.content}</p>
                </section>
              ))}
            </div>
          ) : null}
