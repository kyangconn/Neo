...
export function normalizeImageSettings(input: Partial<ImageGenerationSettings>): ImageGenerationSettings {
  return {
    ...
    worldbookReferenceEnabled: input.worldbookReferenceEnabled ?? DEFAULT_IMAGE_GENERATION_SETTINGS.worldbookReferenceEnabled,
...
function insertPlannedMarkers(content: string, plans: Array<{ afterSegment: number; prompt: string }>, maxImages: number) {
...
function formatPlannerWorldbookReferences(references: ImagePlannerWorldbookReference[] = []) {
  ...
  return [
    ...
    ...valid.map((reference, index) => [
      ...
      clipPlannerReference(reference.content, 1200),
    ].join('\n')),
  ].join('\n\n')
}
...
function replacePlaceholders(value: unknown, replacements: Record<string, string | number>): unknown {
  if (typeof value === 'string') {
    return value.replace(/\{\{\s*(prompt|positive_prompt|positivePrompt|positive|negative_prompt|negativePrompt|negative|seed|width|height|steps|cfg|cfgScale|sampler|samplerName|sampler_name|scheduler|denoise)\s*\}\}/gi, (_match, key: string) => {
      const normalized = key.toLowerCase()
      if (normalized === 'positive' || normalized === 'positive_prompt' || normalized === 'positiveprompt') return String(replacements.prompt)
      if (normalized === 'negative' || normalized === 'negative_prompt' || normalized === 'negativeprompt') return String(replacements.negativePrompt)
      if (normalized === 'cfgscale') return String(replacements.cfg)
      if (normalized === 'sampler' || normalized === 'samplername' || normalized === 'sampler_name') return String(replacements.samplerName)
...
function autoFillCommonWorkflowNodes(workflow: Record<string, any>, prompt: string, settings: ImageGenerationSettings, seed: number) {
  const clipNodes = Object.values(workflow)
    .filter((node: any) => String(node?.class_type || '').toLowerCase().includes('cliptextencode') && node?.inputs && typeof node.inputs.text === 'string')
...
function buildComfyPrompt(workflowJson: string, prompt: string, settings: ImageGenerationSettings) {
  ...
  const seed = settings.seedMode === 'fixed'
    ? settings.fixedSeed
...
async function fetchComfyImageDataUrlDirect(baseUrl: string, image: { filename: string; subfolder?: string; type?: string }, signal?: AbortSignal) {
...
async function getComfyImageDataUrl(baseUrl: string, image: { filename: string; subfolder?: string; type?: string }, signal?: AbortSignal) {
  throwIfAborted(signal)
...
async function queueComfyPrompt(baseUrl: string, workflow: Record<string, any>, clientId: string, signal?: AbortSignal) {
  throwIfAborted(signal)
