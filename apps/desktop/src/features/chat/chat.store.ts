...
import { chatMemoryRepository, chatRepository, chatSavepointRepository, messageRepository, secondaryApiUsageRepository } from '@/db/repositories'
...
interface ChatState {
  ...
  patchMessage: (
    id: string,
    patch: Partial<Pick<Message, 'content' | 'reasoningContent' | 'generateDuration' | 'thinkingDuration' | 'usage' | 'images'>>,
...
function legacyGenerationSnapshot(activeGenerations: Record<string, ActiveGenerationState>, preferredChatId?: string | null) {
  const preferred = preferredChatId ? activeGenerations[preferredChatId] : null
  const nextChatId = preferred
    ? preferredChatId!
    : Object.keys(activeGenerations)[0] ?? null
...
function hydrateMessages(
  ...
  const promise = messageRepository.listByChatId(chatId)
    .then((fullMessages) => {
      const state = get()
...
export const useChatStore = create<ChatState>((set, get) => ({
  ...
  deleteChat: async (id: string) => {
    ...
    try {
      ...
      set((state) => {
        const activeGenerations = Object.fromEntries(Object.entries(state.activeGenerations).filter(([chatId]) => chatId !== id))
        return {
  ...
  beginSending: (chatId: string) => set((state) => {
  ...
  setStreamingMessageId: (chatId: string, id: string | null) => set((state) => {
  ...
  setGenerationPhase: (chatId: string, phase: GenerationPhase) => set((state) => {
    const current = state.activeGenerations[chatId]
    if (!current) return {}
    const activeGenerations = {
      ...state.activeGenerations,
  ...
  finishSending: (chatId?: string) => set((state) => {
    if (!chatId) return { activeGenerations: {}, sending: false, sendingChatId: null, streamingMessageId: null, generationPhase: null }
  ...
  setSending: (sending: boolean) => set((state) => {
    if (!sending) {
      return { activeGenerations: {}, sending: false, sendingChatId: null, streamingMessageId: null, generationPhase: null }
    }
    const chatId = state.sendingChatId ?? state.currentChat?.id
    if (!chatId) {
      return { sending: true, sendingChatId: null, streamingMessageId: state.streamingMessageId, generationPhase: state.generationPhase ?? 'thinking' }
    }
    const activeGenerations = {
      ...state.activeGenerations,
