...
- **严禁单汉字关键词**：单个汉字极易误触发（如"萤"会匹配所有含"萤"字的句子）。昵称必须用两个以上汉字："樱"→"小樱"、"萤"→"小萤"、"玲"→"小玲"或"阿玲"、"奈"→"奈奈"、"樱"→" Sakura"
- **避免过于泛用的词汇**：职务类必须足够精确，如"老师"→"语文老师"、"会长"→"学生会长"
- **避免成语/常见组合**：如"倾国倾城""闭月羞花"等成语作为关键词会导致无关触发
- **角色名本身不受限**：三字全名（如"桃宫樱"、"星野萤"）是安全的，不需要拆分
...
**重要**：每个条目写完后必须立即注册，不得积攒。断点续接（`references/resume.md`）完全依赖 entryManifest 判断进度，一旦漏注册，下次续接时已写条目将被视为未完成，导致重复劳动。
