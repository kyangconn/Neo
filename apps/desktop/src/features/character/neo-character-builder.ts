...
function normalizePlanCharacters(
  value: unknown,
  fallbackText?: string,
  palette?: NeoPersonalityPalette,
): NeoCreationPlan["characters"] {
  if (Array.isArray(value)) {
...
function normalizeCreationPlan(args: Record<string, unknown>, existing?: NeoCreationPlan | null): NeoCreationPlan {
  ...
  const firstMessage =
    args.firstMessage && typeof args.firstMessage === "object" ? (args.firstMessage as Record<string, unknown>) : {};
  ...
  const planBase: Omit<NeoCreationPlan, "yaml" | "updatedAt"> = {
    ...
    world: {
      ...
      regions: normalizeStringList(world.regions).length
        ? normalizeStringList(world.regions)
        : existing?.world?.regions,
      factions: normalizeStringList(world.factions).length
        ? normalizeStringList(world.factions)
        : existing?.world?.factions,
    },
    ...
    firstMessage: {
      ...
      scene:
        optionalString(firstMessage.scene) || optionalString(args.firstMessagePlan) || existing?.firstMessage?.scene,
      openingSituation:
        optionalString(firstMessage.openingSituation || firstMessage.opening_situation) ||
        existing?.firstMessage?.openingSituation,
      wordCount: optionalString(firstMessage.wordCount || firstMessage.word_count) || existing?.firstMessage?.wordCount,
    },
    openQuestions: normalizeStringList(args.openQuestions).length
      ? normalizeStringList(args.openQuestions)
...
function updatePlanEntryStatus(
  plan: NeoCreationPlan | null | undefined,
  args: Record<string, unknown>,
): NeoCreationPlan | undefined {
  if (!plan) return undefined;
  const status: NeoCreationPlanEntry["status"] =
    args.status === "planned" || args.status === "in_progress" || args.status === "done" || args.status === "skipped"
      ? args.status
      : "planned";
...
function buildStopForUserContent(output: Record<string, unknown>) {
  ...
  if (summaryText) {
    return [summaryText, trimString(output.question), optionalString(output.reason)].filter(Boolean).join("\n\n");
  }
  return [trimString(output.question), optionalString(output.reason)].filter(Boolean).join("\n\n");
}
...
): NeoBuilderEvaluationReport["issues"] {
  if (Array.isArray(value)) {
    const issues = value
...
function normalizeEvaluationReport(
  ...
  return {
    summary:
      trimString(args.summary) || (issues.length ? "草稿还有需要修正的地方。" : "草稿结构完整，可以继续细化或保存。"),
    issues,
...
export async function runNeoCharacterBuilderTurn(options: NeoBuilderTurnOptions): Promise<NeoBuilderTurnResult> {
  ...
  for (let round = 0; round < BUILDER_MAX_TOOL_ROUNDS; round++) {
    const result = await generateBuilderStep(
      provider,
      {
        messages,
    ...
    if (result.toolCalls?.length) {
      ...
      if (savedDraft) {
        assistantContent =
          result.content ||
          `产出物已准备好：${savedDraft.character.name}。你可以在右侧查看角色卡与世界书细则，然后保存。`;
        break;
      }
      continue;
    }
  ...
  return {
    content:
      assistantContent || (savedDraft ? `产出物已准备好：${savedDraft.character.name}。` : "我已经处理完这一轮。"),
    choices: pendingChoices,
    draft: savedDraft,
    creationPlan: currentCreationPlan,
        ...
        if (toolName === "list_skill_references") {
          ...
          output = currentCreationPlan
            ? {
                ...
                summary: {
                  status: args.status,
                  done: currentCreationPlan.entries.filter((entry) => entry.status === "done").length,
                  total: currentCreationPlan.entries.length,
                },
              }
            : { ok: false, error: "当前没有可更新的创作规划。" };
          ...
          output = {
            ok: true,
            evaluationReport,
            summary: { issues: evaluationReport.issues.length, score: evaluationReport.score },
          };
